...
def upload_to_imgbb(data, name):
    print(f"   [ImgBB] 正在上传 {name[:15]}...")
    try:
        files = {"image": (name, data)}
        # verify=False 仅用于测试环境解决SSL问题
        resp = requests.post("https://api.imgbb.com/1/upload", 
                           data={"key": IMGBB_API_KEY}, files=files, timeout=45, verify=False)
        
        if resp.status_code != 200:
            print(f"   ❌ [ImgBB] 失败: HTTP {resp.status_code} - {resp.text[:50]}")
            return {"success": False, "service": "ImgBB", "error": f"HTTP {resp.status_code}"}
        
        res = resp.json()
        if res.get("success"):
            print("   ✅ [ImgBB] 成功")
            return {"success": True, "service": "ImgBB", "url": res["data"]["url"]}
        else:
            err = res.get("error", {}).get("message", "API Error")
            print(f"   ❌ [ImgBB] API拒绝: {err}")
            return {"success": False, "service": "ImgBB", "error": err}
    except Exception as e:
        print(f"   ❌ [ImgBB] 异常: {e}")
        return {"success": False, "service": "ImgBB", "error": "网络错误"}

def upload_to_minio(data, name, fhash):
    print(f"   [MyCloud] 正在上传 {name[:15]}...")
    try:
        ...
        if ext.lower() in [".jpg", ".jpeg"]: ctype = "image/jpeg"
        elif ext.lower() == ".png": ctype = "image/png"
        elif ext.lower() == ".gif": ctype = "image/gif"

        s3.put_object(Bucket=MINIO_BUCKET_NAME, Key=key, Body=data, ContentType=ctype)
        url = f"{MINIO_ENDPOINT}/{MINIO_BUCKET_NAME}/{key}"
        print("   ✅ [MyCloud] 成功")
        return {"success": True, "service": "MyCloud", "url": url}
    except Exception as e:
        print(f"   ❌ [MyCloud] 错误: {e}")
        return {"success": False, "service": "MyCloud", "error": str(e)}
...
@app.post("/upload")
async def upload_endpoint(file: UploadFile = File(...), services: str = Form("myminio")):
    ...
    try:
        ...
        for k in todo:
            func = SERVICE_MAP[k]
            if k == "imgbb": res = func(content, file.filename)
            else: res = func(content, file.filename, fhash)

            if res["success"]: success_list.append(res)
            else: failed_list.append({"service": k, "error": res.get("error")})
...
@app.post("/validate")
async def val(d: dict): return {"success": True}
