...
async def change_username(
    ...
    # 返回新的 Token
    from ..routers.auth import ACCESS_TOKEN_EXPIRE_MINUTES  # 导入常量
    # 最好将常量移到 config.py 统一管理
    # 这里先使用硬编码的备用值
    
    access_token_expires = timedelta(minutes=60 * 24 * 30)  # 30天过期
...
def get_history(
    ...
    # [FIX] 对于未登录用户，从 Cookie 获取 device_id，用于"只看我的"过滤
    device_id = request.cookies.get("device_id") if not user_id else None

    final_view_mode = view_mode
    
    # [Admin] 管理员上帝模式
    if view_mode == "admin_all":
        if not is_admin:
             # 如果不是管理员即使请求了也没用，强制降级
             final_view_mode = "private" 
        else:
             final_view_mode = "admin_all"

    elif not user_id:
        final_view_mode = "shared"
    
    return database.get_history_list(page, page_size, keyword, device_id=device_id, user_id=user_id, is_admin=is_admin, view_mode=final_view_mode, only_mine=only_mine)
...
async def generate_vip_codes(
    request: schemas.GenerateVipCodesRequest,
    current_user: dict = Depends(get_current_user)
):
    """(Admin) 批量生成 VIP 激活码并下载"""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="需要管理员权限")
    ...
    def generate_code_str(length=16):
    ...
    # 防止滥用，限制单次最大生成数量
    if count > 10000:
         raise HTTPException(status_code=400, detail="单次最多生成 10000 个")
    
    for _ in range(count):
        code = generate_code_str()
        if database.create_vip_code(code, days):
            codes.append(code)
            
    # 生成文件内容
    output = f"# Batch Generated by Admin ({current_user['username']})\n"
    output += f"# Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    output += f"# Count: {len(codes)}\n"
    output += f"# Days: {days}\n\n"
    output += "\n".join(codes)
    
    stream = io.BytesIO(output.encode('utf-8'))
    
    return StreamingResponse(
        stream,
        media_type="text/plain",
        headers={"Content-Disposition": f"attachment; filename=vip_codes_{len(codes)}_{days}days.txt"}
    )
